<div style="position: relative; height:360px; width: 100%;" class="product-container text-center box-sd">
    <div style="height: 260px; display: flex; padding-top: 20px;" class="image">
        <img class="img-responsive" src="<?= $product->getFeaturedImage() ?>" alt=""
            style="margin: auto; width: auto; max-height: 245px;">
    </div>
    <div class="product-meta">
        <h5 style="min-height: 32px; display: flex;" class="name">
            <a style="margin: auto; padding: 0 12px;" class="product-name"
                href="index.php?c=product&a=show&id=<?= $product->getId() ?>"
        ...
        <div class="product-item-price">
            ...
            <?php endif ?>
            <span
                class="product-item-discount"><?= number_format($product->getPrice()) == 0 ? "không có dữ liệu" : number_format($product->getPrice()) ?>₫</span>
            <?php if ($product->getInventoryQty() > 0): ?>
            <div class="box box-stock">
                <img class="icon1" src="../upload/iconstock.png" alt="">
                <p style="margin-bottom: 0;padding-right: 8px;">Còn <?php echo $product->getInventoryQty() ?> sản phẩm
                </p>
            </div>
            <?php else: ?>
